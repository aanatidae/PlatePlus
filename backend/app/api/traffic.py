...
def _settings(database: Session) -> TrafficSimulationSettings:
    settings = database.scalar(
        select(TrafficSimulationSettings).where(TrafficSimulationSettings.singleton_key == "default")
    )
    if settings is None:
        raise HTTPException(status_code=503, detail="Traffic simulation is not initialized. Run migrations.")
    return settings


def _read_settings(settings: TrafficSimulationSettings) -> SimulationSettingsRead:
    return SimulationSettingsRead(
        **{field: getattr(settings, field) for field in SimulationSettingsRead.model_fields if field != "current_simulation_time"},
...
def _audit(database: Session, admin: Admin, action: str, entity_type: str, details: dict) -> None:
    database.add(AdminAuditLog(admin_id=admin.id, action=action, entity_type=entity_type, details_json=json.dumps(details, default=str)))
...
def update_settings(payload: SimulationSettingsUpdate, database: DatabaseSession, admin: CurrentAdmin):
    ...
    _audit(database, admin, "traffic_settings_updated", "traffic_simulation_settings", payload.model_dump(mode="json"))
    if payload.is_enabled and not was_enabled:
        result = run_simulation(database, settings, source="scheduled")
        _audit(database, admin, "traffic_simulation_run", "traffic_record", {"source": "scheduled", "traffic_record_id": result.traffic_record.id})
    database.commit()
...
def list_pricing_rules(database: DatabaseSession):
    return list(database.scalars(select(DynamicPricingRule).order_by(DynamicPricingRule.minimum_percentage)))


@router.put("/pricing-rules", response_model=list[PricingRuleRead])
def update_pricing_rules(payload: PricingRulesUpdate, database: DatabaseSession, admin: CurrentAdmin):
    ...
    if set(rules) != {item.scenario for item in payload.rules}:
        raise HTTPException(status_code=503, detail="Dynamic pricing rules are not initialized. Run migrations.")
    ...
    if latest_traffic is not None:
        matching = next(item for item in payload.rules if item.minimum_percentage <= latest_traffic.congestion_percentage <= item.maximum_percentage)
        database.add(
            TollPrice(
    ...
    _audit(database, admin, "pricing_rules_updated", "dynamic_pricing_rules", payload.model_dump(mode="json"))
    database.commit()
    return list(database.scalars(select(DynamicPricingRule).order_by(DynamicPricingRule.minimum_percentage)))
...
def list_audit_logs(database: DatabaseSession):
    return list(database.scalars(select(AdminAuditLog).order_by(AdminAuditLog.created_at.desc()).limit(100)))
