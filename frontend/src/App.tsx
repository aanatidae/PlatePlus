...
function SimulatorPage() {
  const [live, setLive] = useState<Overview | null>(null); const [scenario, setScenario] = useState<SandboxScenario>("moderate"); const [custom, setCustom] = useState({ congestion: 55, lanes: 3, base: 3.5 }); const [result, setResult] = useState<SandboxRun | null>(null); const [history, setHistory] = useState<SandboxRun[]>([]);
  ...
  const source = scenario === "custom" ? { title: "Custom scenario", volume: Math.round(900 + custom.congestion * 42), speed: Math.round(Math.max(18, 82 - custom.congestion * .62)), congestion: custom.congestion, description: "Operator configured" } : presets[scenario];
  const base = Number(live?.live.price?.base_amount ?? custom.base ?? 2); const fixedTolls: Record<Exclude<SandboxScenario, "custom">, number> = { normal: 2, moderate: 3, peak_hour: 4, severe: 5 }; const multiplier = source.congestion >= 84 ? 1.5 : source.congestion >= 61 ? 1.25 : source.congestion >= 31 ? 1.1 : 1; const recommended = scenario === "custom" ? Number((base * multiplier).toFixed(2)) : fixedTolls[scenario];
  ...
  function reset() { setScenario("moderate"); setCustom({ congestion: 55, lanes: 3, base: 3.5 }); setResult(null); }
