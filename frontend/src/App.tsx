...
function AdminShell({ admin, route, onLogout }: { admin: Admin; route: string; onLogout: () => void }) {
  ...
  const page = route === "/webcam" ? <WebcamPage /> : route === "/simulator" ? <SimulatorPage /> : route === "/recognition" ? <RecognitionPage key={selected} /> : route === "/pricing" ? <TrafficPage /> : route === "/intelligence" ? <IntelligencePage key={selected} /> : <NetworkOverview />;
...
type IntelligenceSummary = { thresholds: { detection: number; ocr: number }; evaluation: { detector: { accuracy_percent: number; precision: number | null; recall: number | null; f1: number | null; note: string }; ocr: { exact_matches: number; held_out_samples: number; exact_match_accuracy_percent: number; note: string } }; charge_eligibility: string; outcome_analysis: { accepted: number; low_confidence: number; unknown_vehicle: number; errors: number; false_positives: number | null; false_negatives: number | null; note: string }; alpr_trace: { detected_at: string; source: string; stages: { name: string; detail: string; value?: number | null; threshold?: number; result: string }[] } | null; pricing_trace: { location_name: string; telemetry_source: string; traffic: { congestion_percentage: number; congestion_category: string; vehicle_count: number; road_capacity: number } | null; policy: { rule_version: number; base_toll: number; minimum_toll: number; maximum_multiplier: number; minimum_change_minutes: number; hysteresis_percentage: number; band: string | null; band_multiplier: number | null }; resulting_toll: number | null } | null; known_failure_conditions: string[] };
...
function IntelligencePage() {
  ...
  const intelligenceFeed = useFeed<IntelligenceSummary>(locationPath("/api/intelligence/summary", selected));
  const settingsFeed = useFeed<TrafficSettings>("/api/traffic/settings");
  const intelligence = intelligenceFeed.data; const settings = settingsFeed.data;
  const error = intelligenceFeed.error || settingsFeed.error;
  ...
  const threshold = (value: number | null | undefined) => value == null ? "—" : `${(value * 100).toFixed(0)}%`;
  const alpr = intelligence?.alpr_trace;
  const pricing = intelligence?.pricing_trace;
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">EXPLAINABLE AI</p><h1>AI intelligence</h1><p>Evidence, safeguards, and decision paths for PlatePlus’s local ALPR and simulated rule-based toll policy.</p></div><span className="refresh-note"><span className="live-dot" /> {error ? "Telemetry reconnecting" : "Decision trace online"}</span></section>{error && <p className="traffic-notice">{error} Previously loaded evidence remains visible where available.</p>}<section className="metric-grid page-metrics"><Metric tone="teal" label="Detection gate" value={threshold(intelligence?.thresholds.detection)} detail="YOLO candidate-region minimum" /><Metric tone="teal" label="OCR gate" value={threshold(intelligence?.thresholds.ocr)} detail="PaddleOCR charge-safety minimum" /><Metric label="Traffic model" value={trafficModel} detail={settings ? (settings.is_enabled ? `Scheduled every ${settings.interval_minutes} min` : "Manual simulation enabled") : "Simulated traffic model"} /><Metric label="Simulation clock" value={settings?.time_mode === "simulated" ? "Simulated time" : "Malaysia time"} detail={clockDetail} /></section><section className="pipeline-card"><div className="section-title"><div><p className="eyebrow">DECISION PIPELINE</p><h2>How a plate reaches a simulated toll outcome</h2></div></div><ol className="pipeline"><li><span>01</span><strong>Local input</strong><small>Image bytes remain on the operator device.</small></li><li><span>02</span><strong>Detection</strong><small>YOLO must meet the active detection threshold.</small></li><li><span>03</span><strong>OCR + normalize</strong><small>PaddleOCR must meet its gate before a plate is retained.</small></li><li><span>04</span><strong>Vehicle match</strong><small>A registered vehicle is required for charging.</small></li><li><span>05</span><strong>Simulated payment</strong><small>Account and duplicate checks determine the final outcome.</small></li></ol></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">EVALUATION EVIDENCE</p><h2>Detection and OCR are measured separately</h2></div><span>Held-out evidence</span></div><div className="rule-grid"><article className="price-rule normal"><span>YOLO detector</span><strong>{intelligence ? `${intelligence.evaluation.detector.accuracy_percent.toFixed(1)}% accuracy` : "Loading…"}</strong><small>{intelligence?.evaluation.detector.note}</small></article><article className="price-rule"><span>Detection precision</span><strong>{intelligence?.evaluation.detector.precision == null ? "Not recorded" : `${intelligence.evaluation.detector.precision.toFixed(1)}%`}</strong><small>Not inferred from reported accuracy.</small></article><article className="price-rule"><span>Detection recall / F1</span><strong>{intelligence?.evaluation.detector.recall == null ? "Not recorded" : `${intelligence.evaluation.detector.recall.toFixed(1)}% / ${intelligence.evaluation.detector.f1?.toFixed(1)}%`}</strong><small>Export evaluation artifacts to populate these metrics.</small></article><article className="price-rule normal"><span>PaddleOCR exact match</span><strong>{intelligence ? `${intelligence.evaluation.ocr.exact_match_accuracy_percent.toFixed(1)}%` : "Loading…"}</strong><small>{intelligence ? `${intelligence.evaluation.ocr.exact_matches} / ${intelligence.evaluation.ocr.held_out_samples} preserved held-out crops` : ""}</small></article></div></section><section className="trace-grid"><section className="detail-card"><div className="section-title"><div><p className="eyebrow">LATEST ALPR TRACE</p><h2>{alpr ? `Recorded ${new Date(alpr.detected_at).toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}` : "No recorded recognition"}</h2></div><span>{alpr?.source ?? "Awaiting input"}</span></div>{alpr ? <ol className="decision-trace">{alpr.stages.map((stage, index) => <li key={stage.name}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{stage.name}</strong><small>{stage.detail}</small>{stage.value != null && <small>Evidence {threshold(stage.value)}{stage.threshold != null ? ` · gate ${threshold(stage.threshold)}` : ""}</small>}</div><em>{stage.result}</em></li>)}</ol> : <p className="field-note">Run or record an ALPR event to inspect each recognition, normalization, match, and payment decision.</p>}</section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">PRICING TRACE</p><h2>{pricing ? pricing.location_name : "Select one toll location"}</h2></div><span>{pricing?.telemetry_source ?? "Network scope"}</span></div>{pricing?.traffic ? <ol className="decision-trace"><li><span>01</span><div><strong>Traffic input</strong><small>{Number(pricing.traffic.vehicle_count).toLocaleString()} / {Number(pricing.traffic.road_capacity).toLocaleString()} vehicles/hour</small></div><em>{Number(pricing.traffic.congestion_percentage).toFixed(1)}%</em></li><li><span>02</span><div><strong>Congestion result</strong><small>Profile-derived percentage maps to the configured band.</small></div><em>{categoryLabel(pricing.traffic.congestion_category)}</em></li><li><span>03</span><div><strong>Rule-based policy</strong><small>v{pricing.policy.rule_version} · RM{Number(pricing.policy.base_toll).toFixed(2)} × {Number(pricing.policy.band_multiplier ?? 1).toFixed(2)}; hysteresis {Number(pricing.policy.hysteresis_percentage).toFixed(1)}%</small></div><em>{pricing.policy.band ?? "—"}</em></li><li><span>04</span><div><strong>Resulting simulated toll</strong><small>Floor RM{Number(pricing.policy.minimum_toll).toFixed(2)} · cooldown {pricing.policy.minimum_change_minutes} min</small></div><em>RM{Number(pricing.resulting_toll ?? 0).toFixed(2)}</em></li></ol> : <p className="field-note">Choose an individual location in the top bar to inspect its current traffic-to-price decision.</p>}</section></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">SAFETY AND LIMITS</p><h2>Charge eligibility and known failure conditions</h2></div></div><p className="field-note">{intelligence?.charge_eligibility ?? "Loading active eligibility policy…"}</p><ul className="failure-list">{intelligence?.known_failure_conditions.map(item => <li key={item}>{item}</li>)}</ul><p className="field-note">Dynamic pricing is a simulated, configurable rule-based policy—not a prediction model, real toll policy, or official traffic feed.</p></section></main>;
}
