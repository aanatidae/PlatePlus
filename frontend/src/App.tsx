...
import { createSimulationFrames, createSimulatorOutput, scenarioDetails, scenarioTitle, type SimulatorFrame, type SimulatorScenario } from "./simulator";
...
function SimulatorPage() {
  ...
  useEffect(() => {
    if (!batch || batch.frames.length < 2) return;
    const timer = window.setInterval(() => setPlayhead(previous => previous >= batch.frames.length - 1 ? 0 : previous + 1), Math.max(350, 1100 / playbackSpeed));
