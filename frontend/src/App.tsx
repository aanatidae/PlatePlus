...
function OverviewPage() {
  ...
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">REAL-TIME TELEMETRY</p><h1>Live toll operations overview</h1><p>What is happening now across traffic flow, recognition, and toll operations.</p></div><div className="refresh-note"><span className="live-dot" /> LIVE DATA<br /><strong>{new Intl.DateTimeFormat("en-MY", { timeZone: "Asia/Kuala_Lumpur", hour: "2-digit", minute: "2-digit", hour12: true }).format(clock)}</strong></div></section>
...
function RecognitionPage() {
  const [detections, setDetections] = useState<Detection[]>([]); const [transactions, setTransactions] = useState<Transaction[]>([]); const [error, setError] = useState(""); const [uploadResult, setUploadResult] = useState<FrameResult | null>(null); const [uploading, setUploading] = useState(false);
  async function load() { const [detectionsResponse, transactionsResponse] = await Promise.all([fetch(`${API_BASE_URL}/api/data/detections?limit=50`, { headers: apiHeaders() }), fetch(`${API_BASE_URL}/api/data/transactions?limit=50`, { headers: apiHeaders() })]); if (!detectionsResponse.ok || !transactionsResponse.ok) throw new Error("Recognition records are unavailable."); setDetections(await detectionsResponse.json() as Detection[]); setTransactions(await transactionsResponse.json() as Transaction[]); }
  useEffect(() => { void load().catch((reason) => setError(reason instanceof Error ? reason.message : "Recognition records are unavailable.")); }, []);
  async function submitImage(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const image = new FormData(event.currentTarget).get("image"); if (!(image instanceof File) || image.size === 0) { setError("Choose a JPEG, PNG, or WebP image first."); return; } setError(""); setUploading(true); try { const form = new FormData(); form.append("image", image); const response = await fetch(`${API_BASE_URL}/api/webcam/images`, { method: "POST", headers: { ...apiHeaders(), "Idempotency-Key": crypto.randomUUID() }, body: form }); const body = await response.json().catch(() => null) as FrameResult | { detail?: string } | null; if (!response.ok) throw new Error(body && "detail" in body ? body.detail : "Image recognition failed."); setUploadResult(body as FrameResult); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Image recognition failed."); } finally { setUploading(false); } }
  if (error) return <PageError message={error} />;
  const accepted = detections.filter((item) => item.status === "accepted").length;
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">ALPR OPERATIONS</p><h1>Plate recognition</h1><p>Upload a still image for local-only ALPR, or review persisted simulated results. Image bytes are never retained.</p></div><span className="refresh-note"><span className="live-dot" /> Protected data feed</span></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">STILL-IMAGE TEST</p><h2>Run local ALPR once</h2></div><span>JPEG, PNG, or WebP · max 5 MB</span></div><form className="upload-form" onSubmit={(event) => void submitImage(event)}><input name="image" type="file" accept="image/jpeg,image/png,image/webp" required /><button disabled={uploading}>{uploading ? "Processing locally…" : "Recognize image"}</button></form>{uploadResult && <p className="traffic-notice"><strong>{uploadResult.plate_text ?? "No plate read"}</strong> · {uploadResult.message}{uploadResult.payment_status ? ` Simulated payment: ${uploadResult.payment_status}.` : ""}</p>}</section><section className="metric-grid page-metrics"><Metric tone="teal" label="Total detections" value={String(detections.length)} detail="Latest 50 records" /><Metric tone="teal" label="Accepted reads" value={String(accepted)} detail="Passed confidence gate" /><Metric label="Manual review" value={String(detections.length - accepted)} detail="Low confidence or unmatched" /><Metric label="Payments linked" value={String(transactions.length)} detail="Latest 50 transactions" /></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">DETECTION LOG</p><h2>Latest roadside recognitions</h2></div><span>Malaysia time</span></div><div className="data-table-wrap"><table><thead><tr><th>Plate</th><th>Recognition</th><th>Detection</th><th>Vehicle</th><th>Captured</th></tr></thead><tbody>{detections.length ? detections.map((item) => <tr key={item.id}><td><strong>{item.normalized_plate ?? "Unread plate"}</strong></td><td><span className={`status ${item.status}`}>{item.status.replace(/_/g, " ")}</span></td><td>{(Number(item.ocr_confidence ?? item.detection_confidence) * 100).toFixed(1)}%</td><td>{item.vehicle_id ? "Registered" : "Unknown"}</td><td>{new Date(item.detected_at).toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}</td></tr>) : <tr><td colSpan={5}>No simulated detection records are available.</td></tr>}</tbody></table></div></section></main>;
}
...
function IntelligencePage() {
  const [overview, setOverview] = useState<Overview | null>(null); const [settings, setSettings] = useState<TrafficSettings | null>(null); const [error, setError] = useState(""); const [clock, setClock] = useState(new Date());
...
function SimulatorPage() {
  const [live, setLive] = useState<Overview | null>(null); const [scenario, setScenario] = useState<SandboxScenario>("moderate"); const [custom, setCustom] = useState({ congestion: 2800, lanes: 3, base: 2 }); const [result, setResult] = useState<SandboxRun | null>(null); const [history, setHistory] = useState<SandboxRun[]>([]);
  ...
  const customVolume = Math.min(5000, Math.max(0, custom.congestion)); const customCongestion = Number(((customVolume / 5000) * 100).toFixed(2)); const source = scenario === "custom" ? { title: "Custom scenario", volume: customVolume, speed: Math.round(Math.max(18, 82 - customCongestion * .62)), congestion: customCongestion, description: "Operator configured" } : presets[scenario];
  const base = scenario === "custom" ? custom.base : 2; const fixedTolls: Record<Exclude<SandboxScenario, "custom">, number> = { normal: 2, moderate: 3, peak_hour: 4, severe: 5 }; const priceIncrease = source.congestion > 80 ? 3 : source.congestion > 60 ? 2 : source.congestion > 30 ? 1 : 0; const multiplier = base ? Number(((base + priceIncrease) / base).toFixed(2)) : 1; const recommended = scenario === "custom" ? Number((base + priceIncrease).toFixed(2)) : fixedTolls[scenario];
  ...
  function reset() { setScenario("moderate"); setCustom({ congestion: 2800, lanes: 3, base: 2 }); setResult(null); }
