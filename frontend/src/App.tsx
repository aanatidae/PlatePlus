...
function OverviewPage() {
  ...
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">PENCHALA TOLL PLAZA · LDP / E11</p><h1>Traffic operations overview</h1><p>All traffic, pricing, and payments shown here are simulated.</p></div><span className="refresh-note">Refreshes every 30 seconds · Malaysia time</span></section>
    ...
    <section className="metric-grid"><Metric label="Current toll" value={data?.live.price ? `RM${Number(data.live.price.amount).toFixed(2)}` : "N/A"} /><Metric label="Detections" value={String(data?.metrics.detections ?? 0)} /><Metric label="Transactions" value={String(data?.metrics.transactions ?? 0)} /><Metric label="Simulated revenue" value={`RM${Number(data?.metrics.revenue ?? 0).toFixed(2)}`} /><Metric label="Recognition confidence" value={data?.metrics.average_recognition_confidence ? `${(Number(data.metrics.average_recognition_confidence) * 100).toFixed(1)}%` : "N/A"} /></section>
