...
function SimulatorPage() {
  const [live, setLive] = useState<Overview | null>(null); const [scenario, setScenario] = useState<SandboxScenario>("moderate"); const [custom, setCustom] = useState({ volume: 2800, congestion: 0, lanes: 3, base: 2 }); const [result, setResult] = useState<SandboxRun | null>(null); const [history, setHistory] = useState<SandboxRun[]>([]);
  ...
  const customCongestion = Math.min(100, Number(((custom.volume / 5000) * 100).toFixed(2))); const source = scenario === "custom" ? { title: "Custom scenario", volume: custom.volume, speed: Math.round(Math.max(18, 82 - customCongestion * .62)), congestion: customCongestion, description: "Operator configured" } : presets[scenario];
  ...
  function reset() { setScenario("moderate"); setCustom({ volume: 2800, congestion: 0, lanes: 3, base: 2 }); setResult(null); }
