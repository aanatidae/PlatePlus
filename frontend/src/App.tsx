...
import { Activity, BrainCircuit, Camera, CircleDollarSign, Gauge, MapPin, Menu, Radar, RefreshCw, SlidersHorizontal, TrafficCone, X } from "lucide-react";
  ...
  useEffect(() => {
    ...
    void fetch(`${API_BASE_URL}/api/auth/me`, { headers: { Authorization: `Bearer ${session.access_token}` } })
      ...
      .then((admin) => {
        ...
        if (!["/dashboard", "/recognition", "/traffic", "/pricing", "/intelligence", "/webcam"].includes(route)) navigate("/dashboard", true);
      })
      .catch(() => {
        if (cancelled) return;
        window.sessionStorage.removeItem(AUTH_STORAGE_KEY);
...
function AdminShell({ admin, route, onLogout }: { admin: Admin; route: string; onLogout: () => void }) {
  ...
  const page = route === "/webcam" ? <WebcamPage /> : route === "/traffic" ? <TrafficPage /> : route === "/recognition" ? <RecognitionPage /> : route === "/pricing" ? <PricingPage /> : route === "/intelligence" ? <IntelligencePage /> : <OverviewPage />;
  ...
  return <div className="command-shell">
    <aside className={menuOpen ? "command-sidebar open" : "command-sidebar"}>
      ...
      <nav aria-label="Administrator navigation">
        ...
        <NavLink to="/traffic" active={route === "/traffic"} icon={<TrafficCone size={17} />}>Traffic analytics</NavLink>
        ...
        <NavLink to="/traffic" active={route === "/traffic"} icon={<SlidersHorizontal size={17} />}>Simulation</NavLink>
...
function OverviewPage() {
  ...
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">NETWORK COMMAND CENTRE</p><h1>Traffic operations overview</h1><p>Live simulated telemetry for traffic flow, license plate recognition, and toll decisions.</p></div><div className="refresh-note"><span className="live-dot" /> Refreshes every 30 seconds<br />Malaysia time</div></section>
    ...
    <section className="metric-grid"><Metric tone="teal" label="Current toll" value={data?.live.price ? `RM${Number(data.live.price.amount).toFixed(2)}` : "N/A"} detail="AI-adjusted price" /><Metric tone={tone} label="Congestion" value={traffic ? `${traffic.congestion_percentage}%` : "N/A"} detail={traffic?.congestion_category ?? "Awaiting simulation"} /><Metric label="Vehicles detected" value={String(data?.metrics.detections ?? 0)} detail="Current selection" /><Metric label="Toll transactions" value={String(data?.metrics.transactions ?? 0)} detail={`${successRate}% completed`} /><Metric tone="teal" label="Recognition confidence" value={data?.metrics.average_recognition_confidence ? `${(Number(data.metrics.average_recognition_confidence) * 100).toFixed(1)}%` : "N/A"} detail="Average OCR result" /></section>
...
function PricingPage() {
  ...
  useEffect(() => { let cancelled = false; void Promise.all([fetch(`${API_BASE_URL}/api/data/toll-prices?limit=50`, { headers: apiHeaders() }), fetch(`${API_BASE_URL}/api/traffic/pricing-rules`, { headers: apiHeaders() }), fetch(`${API_BASE_URL}/api/dashboard/overview?limit=1`, { headers: apiHeaders() })]).then(async ([pricesResponse, rulesResponse, overviewResponse]) => { if (!pricesResponse.ok || !rulesResponse.ok || !overviewResponse.ok) throw new Error("Dynamic pricing data is unavailable."); return [await pricesResponse.json() as TollPrice[], await rulesResponse.json() as PricingRule[], await overviewResponse.json() as Overview] as const; }).then(([nextPrices, nextRules, nextOverview]) => { if (!cancelled) { setPrices(nextPrices); setRules(nextRules); setOverview(nextOverview); } }).catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "Dynamic pricing data is unavailable."); }); return () => { cancelled = true; }; }, []);
  if (error) return <PageError message={error} />;
  const livePrice = overview?.live.price; const liveTraffic = overview?.live.traffic;
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">PRICING COMMAND</p><h1>Dynamic toll pricing</h1><p>Current price decisions use the active simulated congestion rule and remain fully auditable.</p></div><span className="refresh-note"><span className="live-dot" /> Rule-based simulation</span></section><section className="pricing-hero"><div><span className={`status ${liveTraffic?.congestion_category ?? ""}`}>{liveTraffic?.congestion_category ?? "Awaiting traffic"}</span><strong>{livePrice ? `RM${Number(livePrice.amount).toFixed(2)}` : "N/A"}</strong><p>Current simulated toll price</p></div><dl><div><dt>Current congestion</dt><dd>{liveTraffic ? `${liveTraffic.congestion_percentage}%` : "N/A"}</dd></div><div><dt>Vehicle load</dt><dd>{liveTraffic ? `${liveTraffic.vehicle_count} / ${liveTraffic.road_capacity}` : "N/A"}</dd></div><div><dt>Previous price</dt><dd>{prices[1] ? `RM${Number(prices[1].amount).toFixed(2)}` : "No prior record"}</dd></div></dl></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">ACTIVE BANDS</p><h2>Congestion-to-toll rules</h2></div><a href="/traffic" onClick={(event) => { event.preventDefault(); navigate("/traffic"); }}>Manage simulation</a></div><div className="rule-grid">{rules.map((rule) => <article key={rule.id} className={`price-rule ${rule.congestion_category}`}><span>{rule.congestion_category}</span><strong>RM{Number(rule.amount).toFixed(2)}</strong><small>{rule.minimum_percentage}% to {rule.maximum_percentage}% congestion</small></article>)}</div></section><section className="detail-card"><div className="section-title"><div><p className="eyebrow">PRICE HISTORY</p><h2>Recent price decisions</h2></div></div><div className="data-table-wrap"><table><thead><tr><th>Effective time</th><th>Congestion</th><th>Toll price</th><th>Rule version</th></tr></thead><tbody>{prices.length ? prices.map((price) => <tr key={price.id}><td>{new Date(price.effective_at).toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}</td><td><span className={`status ${price.congestion_category}`}>{price.congestion_category}</span></td><td><strong>RM{Number(price.amount).toFixed(2)}</strong></td><td>{price.rule_version ?? "N/A"}</td></tr>) : <tr><td colSpan={4}>No simulated toll prices are available.</td></tr>}</tbody></table></div></section></main>;
}
...
function IntelligencePage() {
  ...
  useEffect(() => { let cancelled = false; void Promise.all([fetch(`${API_BASE_URL}/api/dashboard/overview?limit=8`, { headers: apiHeaders() }), fetch(`${API_BASE_URL}/api/traffic/settings`, { headers: apiHeaders() })]).then(async ([overviewResponse, settingsResponse]) => { if (!overviewResponse.ok || !settingsResponse.ok) throw new Error("AI decision telemetry is unavailable."); return [await overviewResponse.json() as Overview, await settingsResponse.json() as TrafficSettings] as const; }).then(([nextOverview, nextSettings]) => { if (!cancelled) { setOverview(nextOverview); setSettings(nextSettings); } }).catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "AI decision telemetry is unavailable."); }); return () => { cancelled = true; }; }, []);
  if (error) return <PageError message={error} />;
