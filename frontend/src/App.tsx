  ...
  return <div className="command-shell">
    <aside className={menuOpen ? "command-sidebar open" : "command-sidebar"}>
      ...
      <nav aria-label="Administrator navigation">
        ...
        <NavLink to={overview ? "#recognition" : "/dashboard#recognition"} active={false} icon={<Radar size={17} />}>Plate recognition</NavLink>
        <NavLink to="/traffic" active={route === "/traffic"} icon={<TrafficCone size={17} />}>Traffic analytics</NavLink>
        <NavLink to={overview ? "#pricing" : "/dashboard#pricing"} active={false} icon={<CircleDollarSign size={17} />}>Dynamic pricing</NavLink>
        <NavLink to={overview ? "#intelligence" : "/dashboard#intelligence"} active={false} icon={<BrainCircuit size={17} />}>AI intelligence</NavLink>
...
function NavLink({ to, active, icon, children }: { to: string; active: boolean; icon: React.ReactNode; children: React.ReactNode }) {
  ...
  return <a className={active ? "nav-link active" : "nav-link"} href={to} onClick={(event) => { if (!isAnchor) { event.preventDefault(); navigate(to); } }}>{icon}{children}</a>;
}
