...
function LocationCard({ state, openCamera, highlighted }: { state: LocationState; openCamera: () => void; highlighted: boolean }) {
  ...
  return <article className="location-card"><p className="eyebrow">SELECTED LOCATION</p><h2>{state.location.display_name}</h2><p>{state.location.highway_or_route}</p>
    {telemetry ? <><span className={`status ${telemetry.congestion_category}`}>{categoryLabel(telemetry.congestion_category)} · {Number(telemetry.congestion_percentage).toFixed(1)}%</span>
      ...
      <PricingExplanation telemetry={telemetry} source={state.telemetry_source} compact /><p className="field-note">{state.telemetry_source === "webcam_alpr" ? "Local laptop webcam or uploaded-image ALPR · accepted crossings remain active for 60 seconds; raw images stay ephemeral." : state.telemetry_source === "fallback" ? "Simulated time-profile estimate" : state.telemetry_source === "mixed" ? "Simulated estimate with recorded toll" : "Latest recorded simulation · speed estimated"}</p>{state.location.code === "SIMULATOR" && <div className="simulator-inputs"><button onClick={openCamera}>Open Camera</button><SimulatorImageUpload locationId={state.location.id} /></div>}</> : <p>Telemetry unavailable for this location.</p>}
  </article>;
}
...
export default function NetworkOverview() {
  ...
  useEffect(() => {
    ...
    const detection = data?.detections.items.find(item => (item.source === "webcam" || item.source === "webcam_alpr" || item.source === "uploaded_image") && item.location_id === simulatorState?.location.id);
