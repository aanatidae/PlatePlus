...
export default function NetworkOverview() {
  ...
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">REAL-TIME TELEMETRY</p><h1>{title}</h1><p>Current traffic, recognition and simulated toll operations.</p></div><div className="refresh-note" role="status">{stale ? "STALE / RECONNECTING" : data ? "MONITORING" : "LOADING"}<br />Last updated: {receivedAt ? time(new Date(receivedAt).toISOString()) : "Awaiting data"}</div></section>
    ...
    <section className="detail-card"><div className="section-title"><div><p className="eyebrow">ACTIVE ALERTS</p><h2>Current operational issues</h2></div><div><button className="secondary-button" onClick={() => void alertAction("/api/operations/monitor")}>Refresh alerts</button><button className="secondary-button" onClick={() => void alertAction("/api/operations/alerts/seed-demo")}>Seed demo alerts</button><button className="secondary-button" onClick={() => void alertAction("/api/operations/alerts/reset-demo")}>Reset demo alerts</button></div></div><div className="records">{activeAlerts.length ? activeAlerts.map(alert => <div className="record" key={alert.id}><strong>{alert.title} {alert.source === "demo" && "· Demo"}</strong><span className={`status ${alert.severity}`}>{alert.severity}</span><small>{alert.message}<br />Started {time(alert.started_at)}</small><button className="secondary-button" onClick={() => void alertAction(`/api/operations/alerts/${alert.id}/acknowledge`)}>Acknowledge</button></div>) : <p>No active operational alerts.</p>}</div></section>
