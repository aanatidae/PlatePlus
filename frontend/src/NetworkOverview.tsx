...
function LocationCard({ state, openCamera, phoneConnected, laptopConnected }: { state: LocationState; openCamera: () => void; phoneConnected: boolean; laptopConnected: boolean }) {
  ...
  return <article className="location-card"><p className="eyebrow">SELECTED LOCATION</p><h2>{state.location.display_name}</h2><p>{state.location.highway_or_route}</p>
    {telemetry ? <><span className={`status ${telemetry.congestion_category}`}>{categoryLabel(telemetry.congestion_category)} · {Number(telemetry.congestion_percentage).toFixed(1)}%</span>
      ...
      <p className="field-note">{state.telemetry_source === "webcam_alpr" ? `ALPR-only · ${phoneConnected ? "Phone connected" : laptopConnected ? "Laptop camera connected" : "Camera offline"} · accepted crossings remain active for 60 seconds.` : state.telemetry_source === "fallback" ? "Simulated time-profile estimate" : state.telemetry_source === "mixed" ? "Simulated estimate with recorded toll" : "Latest recorded simulation · speed estimated"}</p>{state.location.code === "SIMULATOR" && <button onClick={openCamera}>Open Camera</button>}</> : <p>Telemetry unavailable for this location.</p>}
  </article>;
}
...
export default function NetworkOverview() {
  ...
  const alerts = useFeed<{ id: string; location_id: string | null; alert_type: string; severity: string; status: string; title: string; message: string; source: string; started_at: string; acknowledged_at: string | null }[]>("/api/operations/alerts");
  ...
  const cameraStatus = useFeed<{ phone_connected: boolean; laptop_connected: boolean }>("/api/webcam/status", selected !== "all" && locations.find(item => item.id === selected)?.code === "SIMULATOR");
  const [cameraSource, setCameraSource] = useState<"chooser" | "laptop" | "phone" | null>(null);
  ...
  return <main className="dashboard-page"><section className="page-heading"><div><p className="eyebrow">REAL-TIME TELEMETRY</p><h1>{title}</h1><p>Current traffic, recognition and simulated toll operations.</p></div><div className="refresh-note" role="status">{stale ? "STALE / RECONNECTING" : data ? "MONITORING" : "LOADING"}<br />Last updated: {receivedAt ? time(new Date(receivedAt).toISOString()) : "Awaiting data"}</div></section>
    ...
    {cameraSource === "chooser" && <div className="camera-source-menu" role="dialog" aria-label="Choose Simulator Toll camera source"><strong>Camera source</strong><p>Choose a local ALPR input. Both use the same Simulator Toll crossing rules.</p><button onClick={() => setCameraSource("laptop")}>Laptop camera</button><button className="secondary-button" onClick={() => setCameraSource("phone")}>Phone camera</button><button className="link-button" onClick={() => setCameraSource(null)}>Cancel</button></div>}
    {cameraSource === "laptop" && <CameraCapture compact source="laptop" onClose={() => setCameraSource(null)} />}
    {cameraSource === "phone" && <div className="phone-connect-card" role="dialog" aria-label="Phone scanner connection"><button className="camera-icon" aria-label="Close phone connection instructions" onClick={() => setCameraSource(null)}>×</button><h2>Connect phone scanner</h2><p>Open this local scanner URL on a phone connected to the same Wi-Fi:</p><code>{`${import.meta.env.VITE_SCANNER_URL ?? location.origin}/scanner`}</code><p>{cameraStatus.data?.phone_connected ? "Phone connected and sending sampled frames." : "Waiting for a phone scanner session. Sign in there with the local admin account, then start the camera."}</p><small>Use a LAN HTTPS URL for mobile browser camera permission; laptop camera remains available if HTTPS is not configured.</small></div>}
  </main>;
}
