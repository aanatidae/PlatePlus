...
- The authenticated `/scanner` phone page is a local-network-only optional camera input. It samples ephemeral frames to the laptop FastAPI ALPR session endpoint; phone and laptop inputs have identical Simulator Toll duplicate, payment, persistence, and 60-second crossing semantics. Do not send phone frames to cloud services. A phone is shown connected only after it actively sends frames. LAN HTTPS is required by mobile browser camera policy; retain laptop-camera fallback when it is unavailable.
