...
- Human review filled the manifest without altering the protected test set. The local development run scored 125/146 OCR exact matches (85.6%) and detected all 150 plate-present images. Treat these as development-only values: there are no negative images, motion-blur OCR has no readable ground truth, unusual-format OCR has no samples, and partial-obstruction has only one scored sample. Do not claim a false-positive rate or complete per-condition coverage until additional human-labelled samples are added.
