...
- `Simulator Toll Plaza` is a special live local-ALPR location on the LDP display route. Its rolling traffic flow, congestion, and dynamic toll derive only from accepted laptop-webcam or local uploaded-still-image ALPR crossings; it must never receive generated/fallback traffic telemetry. Raw frames, uploads, and crops remain local and ephemeral, while only processed detection/payment metadata is persisted.
...
- Simulator Toll Plaza is local-ALPR-only. Its accepted laptop-webcam and uploaded-still-image crossings remain historically persisted but contribute to its live congestion only for a rolling 60-second window at a capacity of 10 crossings. Do not fabricate its speed or send it generated/fallback crossings.
...
- Simulator Toll Plaza accepts two genuine local ALPR inputs: the laptop webcam and a browser-selected JPG/JPEG, PNG, or WebP still image (maximum 5 MB). Both use the same local YOLO/PaddleOCR, validation, duplicate protection, simulated payment, and 60-second/10-crossing telemetry workflow. Uploaded-image records use `uploaded_image`; webcam records use `webcam_alpr`; raw images and crops remain ephemeral.
