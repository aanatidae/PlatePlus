...
if (Listening 5173) { Ok "Frontend already listening on http://localhost:5173 (left untouched)" } else { Write-Host "[PlatePlus] Starting frontend..."; $npm = Get-Command npm.cmd -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty Source; if (!$npm) { Fail "npm.cmd is unavailable. Install the existing Node.js prerequisite before using the demo launcher." }; $process = Start-Process -FilePath $npm -ArgumentList "run", "dev", "--", "--host", "127.0.0.1", "--port", "5173" -WorkingDirectory (Join-Path $root "frontend") -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logDirectory "frontend.out.log") -RedirectStandardError (Join-Path $logDirectory "frontend.err.log") -PassThru; $started += @{ name = "frontend"; pid = $process.Id }; Wait-Port 5173 25 "Dashboard" }
