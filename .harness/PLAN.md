...
- The administrator overview now has Malaysia-time metrics, filtered recent records with load-more, traffic/price charts, and a live simulated Penchala Toll Plaza map. Both the map roads and expanded ERP-style hologram gantry reflect the current congestion color, with restrained vehicle-flow markers, upward-moving gantry vehicles, a scanning beam, and smooth hologram expansion/collapse motion. The local webcam lives at `/webcam`; `/traffic` provides protected scheduling, simulated-clock, pricing-band, manual-run, and audit-history controls.
