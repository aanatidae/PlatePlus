...
- The Vercel SPA configuration, free-tier Render Blueprint, CORS configuration, production webcam feature flag, and deployment runbook are prepared. The free Blueprint omits the paid continuous scheduler; administrators use the existing manual simulation action in `/traffic`. Provisioning the approved Render/Vercel accounts, applying the remote migration/seed, and validating a preview remain external steps.
